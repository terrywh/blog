...
async function latest() {
    const html = await (
        await fetch(`https://github.com/simdutf/simdutf/releases/latest/`)
    ).text();
    const match = /<h1 [^>]+>Version v?([0-9][^<\s]*)/.exec(html);
    if (match === null) {
        throw new Error("unable to detect the latest simdutf version.");
    }
    return [match[1]];
}

async function setup() {
    const setup = Bun.file("simdutf-setup.json");
    let stats;
    try {
    ...
    if (stats === null || Date.now() - stats.mtime.getTime() > 3600 * 1000) {
        ...
        const filename = `simdutf-${version}.tar.gz`;
        await Bun.write(setup, JSON.stringify({ version, filename }));
        const url = `${CONFIG.mirror}https://github.com/simdutf/simdutf/archive/refs/tags/v${version}.tar.gz`;
